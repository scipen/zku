...
template sudoku() {
    ...
    component rp_puzzles[9][9];
    component rp_sols[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rp_puzzles[i][j] = RangeProof(32);
            rp_puzzles[i][j].range[0] <== 0;
            rp_puzzles[i][j].range[1] <== 9;
            rp_puzzles[i][j].in <== puzzle[i][j];
            rp_puzzles[i][j].out === 1;

            rp_sols[i][j] = RangeProof(32);
            rp_sols[i][j].range[0] <== 0;
            rp_sols[i][j].range[1] <== 9;
            rp_sols[i][j].in <== solution[i][j];
            rp_sols[i][j].out === 1;
